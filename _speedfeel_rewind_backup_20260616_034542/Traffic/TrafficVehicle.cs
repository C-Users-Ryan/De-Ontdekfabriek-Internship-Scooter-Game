...
namespace KenyaScooter.Traffic
{
    ...
    public sealed class TrafficVehicle : MonoBehaviour, IRewindable
    {
        ...
        public void Activate(LaneDirection direction, TrafficBehaviourProfile profile, float laneCentre, Vector3 position)
        {
            ...
            // despawned, which silently stalls further spawning. Cap it below the player's
            // base speed, but with a RANDOMISED ceiling so fast cars don't all flatten to the
            // exact same value (which reads as "everyone drives your speed late-game").
            if (Direction == LaneDirection.SameDirection && !isStaticObstacle && WorldSpeed.Instance != null)
                baseSpeed = Mathf.Min(baseSpeed, WorldSpeed.Instance.BaseSpeed * Random.Range(0.6f, 0.88f));
