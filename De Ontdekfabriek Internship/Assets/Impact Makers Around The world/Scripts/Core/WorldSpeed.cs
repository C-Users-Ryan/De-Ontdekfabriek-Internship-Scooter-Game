...
{
    /// <summary>
    /// Owns the world's scroll speed (M1, M2). The scooter never actually drives forward —
    /// the whole world moves past it at this speed instead, and that is what sells the feeling
    /// of riding. Only PlayerController feeds it gas and brake. Anything that needs to take the
    /// speed over, like braking into the checkpoint (Req §3.2), calls BeginOverride so it is not
    ...
    public sealed class WorldSpeed : MonoBehaviour
    {
        ...
        /// <summary>A hazard hit scrubs off some speed (set per-hazard), so the player actually loses momentum instead of sailing through it.</summary>
        private void HandleHazardHit(HazardSpawnConfig definition, float playerKmh, Vector3 position)
        {
            if (definition == null || definition.speedPenalty <= 0f)
                return;
            float floor = config != null ? config.minSpeed : 0f;
            Current = Mathf.Max(floor, Current * (1f - definition.speedPenalty));
        }
