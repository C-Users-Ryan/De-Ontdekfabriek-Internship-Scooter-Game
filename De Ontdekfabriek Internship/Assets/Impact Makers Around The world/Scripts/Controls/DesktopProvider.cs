...
{
    /// <summary>
    /// Desktop test controls (Req §3.1 — the keyboard fallback has to work for PC testing). Keyboard:
    /// A/D or arrows steer, W/Up gas, S/Down brake; held keys ramp the steer value so it feels like
    /// gradual tilt. Mouse buttons copy the tablet's touch zones (left half brake, right half gas) so PC
    /// tests use the same two-zone layout as the tablet. A gamepad (the Bluetooth steering wheel) maps
    /// left stick to steer, right trigger to gas, left trigger to brake. TiltDegrees is faked from the
    /// steer value so Real Rider Mode can be previewed on desktop (M7).
    /// </summary>
