    ...
    public sealed class TrafficSpawner : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            for (int i = TrafficVehicle.Active.Count - 1; i >= 0; i--)
            {
                ...
                if (playerLong - RoadDirection.Longitudinal(vehicle.transform.position) > config.despawnBehindDistance)
                    Despawn(vehicle);
            }
        ...
        {
            for (int i = TrafficVehicle.Active.Count - 1; i >= 0; i--)
                TrafficVehicle.Active[i].SourcePool.Release(TrafficVehicle.Active[i]);
        ...
        private void BuildPools(TrafficVehicle[] prefabs)
        {
            for (int i = 0; i < prefabs.Length; i++)
            {
                ...
                var pool = new ObjectPool<TrafficVehicle>(prefab, transform, config.poolSizePerPrefab,
                    created =>
                    {
