...
namespace KenyaScooter.Scoring
{
    ...
    public sealed class ScoreManager : MonoBehaviour
    {
        ...
        {
            if (streak != null) streak.ResetStreak();
            if (definition == null || !definition.deduct)
                return;

            // Each hazard carries its own deduction now — adding a hazard type is an asset,
            // not another case here (SC4). Speed-scaling and popup key are per-config too.
            int amount = definition.speedScaled ? SpeedScaled(definition.deduction) : definition.deduction;
            string popupKey = string.IsNullOrEmpty(definition.popupKey) ? null : definition.popupKey;
            Deduct(amount, popupKey, position, true);
        }
