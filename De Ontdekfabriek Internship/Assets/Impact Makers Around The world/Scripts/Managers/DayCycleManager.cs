...
namespace OvertakeGame
{
    ...
    ///   - Was: independent timer, cycled Morning→Midday→Sunset→Morning
    ///   - Now: session-driven one-way curve, dawn→sunrise→midday→afternoon→sunset
    ///   - Was: did not set RenderSettings.ambientLight → scene was always dark
    ///   - Now: drives ambient directly per keyframe (this is the main dark-sky fix)
    ///   - Was: only 3 phases, harsh jumps
    ///   - Now: 5 keyframes, always interpolating
    ///   - Was: sun only had 3 angle positions
    ///   - Now: sun rotates continuously east → overhead → west
    ///
    /// SETUP:
