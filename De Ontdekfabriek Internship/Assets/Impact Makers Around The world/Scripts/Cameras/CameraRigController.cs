...
{
    /// <summary>
    /// The ONLY script that writes the camera rig, in LateUpdate (architecture rule,
    /// Req §2). It composes three inputs each frame: rig yaw following
    /// RoadDirection.Current with the 0.35 s turn rotation (M3), the positional shake
    /// offset from CameraShake, and the horizon roll from RealRiderMode (M7). Those
    /// two components only compute values — they never touch a transform.
    /// </summary>
