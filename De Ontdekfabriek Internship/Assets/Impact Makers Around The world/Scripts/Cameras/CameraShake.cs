...
{
    /// <summary>
    /// Works out a Perlin-noise shake offset for collisions, near misses (M18) and hazard hits. It only
    /// exposes CurrentOffset — CameraRigController applies it, which keeps the "one script writes the
    /// camera" rule intact.
    /// </summary>
    public sealed class CameraShake : MonoBehaviour
    {
        ...
        {
            // Per-hazard shake strength (a rock can hit harder than a pothole), scaled up with speed.
            float strength = definition != null && definition.shakeStrength > 0f ? definition.shakeStrength : hazardStrength;
            Impulse(strength * Mathf.Max(0.6f, WorldSpeed.Instance.SpeedRatio), defaultDuration);
        }
