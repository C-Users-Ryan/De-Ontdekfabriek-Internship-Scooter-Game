...
{
    /// <summary>
    /// The rewind (M17, Req §8.2). While playing, a ring buffer records a snapshot of every registered
    /// participant (player, traffic, hazards, tiles) plus the world speed, every rewindSnapshotInterval.
    /// On a hard crash GameManager sets the state to Rewinding and calls BeginRewind: fade to
    /// black-and-white, play the buffered frames in reverse, snap exactly to the oldest one, fade back,
    /// then raise RewindCompleted. The score penalty and streak reset hang off that event in ScoreManager,
    /// and the spawners rebuild their pools from it too. All frame storage is preallocated, so recording
    /// allocates nothing.
    /// </summary>
    public sealed class RewindSystem : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // If no RewindVisuals was wired up in the scene, add one so the screen still greys out on a rewind.
            if (visuals == null)
                visuals = gameObject.AddComponent<RewindVisuals>();
        }
