...
namespace OvertakeGame
{
    ...
    public class TrafficManager : MonoBehaviour
    {
        ...
        private IEnumerator SameLaneRoutine()
        {
            while (true)
            {
                float mult = _profile?.sameDirectionIntervalMult ?? 1f;
        ...
        private IEnumerator OncomingRoutine()
        {
            while (true)
            {
                float mult = _profile?.oncomingIntervalMult ?? 1f;
        ...
        private Vector3 MakeLanePos(bool oncoming, float ahead)
        {
            ...
            Vector3 pos = _player.position
                        + forwardDir               * ahead
                        + RoadDirection.SteerAxis * (sign * laneOffset);
            pos.y = spawnY;
            return pos;
        }
