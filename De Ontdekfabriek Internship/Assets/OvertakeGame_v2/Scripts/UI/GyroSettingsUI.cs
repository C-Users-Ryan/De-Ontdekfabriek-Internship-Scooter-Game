...
namespace OvertakeGame
{
    ...
    public class GyroSettingsUI : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Keyboard.current != null && Keyboard.current[settingsPanelKey].wasPressedThisFrame)
                settingsPanel?.SetActive(!settingsPanel.activeSelf);

            // Quick-toggle Real Rider Mode without opening the panel
            if (Keyboard.current != null && Keyboard.current[realRiderKey].wasPressedThisFrame && gyroSteering != null)
            {
