...
namespace KenyaScooter.Traffic
{
    ...
    public sealed class TrafficVehicle : MonoBehaviour, IRewindable
    {
        ...
        public void Activate(LaneDirection direction, TrafficBehaviourProfile profile, float laneCentre, Vector3 position)
        {
            ...
            // Same-direction traffic must stay slower than the player, or it can never be
            // overtaken — and a vehicle that is never overtaken never falls behind to be
            // despawned, which silently stalls all further same-direction spawning. Cap it
            // just under the player's coasting (base) speed so overtaking and recycling always work.
            if (Direction == LaneDirection.SameDirection && !isStaticObstacle && WorldSpeed.Instance != null)
                baseSpeed = Mathf.Min(baseSpeed, WorldSpeed.Instance.BaseSpeed * 0.9f);
        ...
        public void Tick(float deltaTime, TrafficConfig config, Vector3 playerPosition)
        {
            ...
            if (Direction == LaneDirection.SameDirection && !isStaticObstacle)
            {
                ...
                {
                    // Yield to the player closing in from behind: shift toward the
                    // verge and ease off. Cautious drivers (large yieldShift) make
                    // visibly more room than Aggressive ones (Req §5.1, MDA D4).
                    lateralTarget += Mathf.Sign(laneCentre) * settings.yieldShift;
                    targetSpeed *= Mathf.Lerp(1f, 0.85f, settings.yieldShift);
                }
            }
